...
from beans import Beans, BeansParam
...
def plot_markers(ax, ax2, beans:Beans):
    timestamp = beans.timestamp
    cl = beans.cl
...
def analyze(title, csv_path, graph_height):
    ...
    param.short_term = 13
    param.atr_term = 25
    param.slice_threshold = 0.05
    param.signal_filter_window = 10
    param.signal_filter_nummax = 3
    param.trend_slope_threshold = 1
...
def plot_chart(title, timestamp, beans:Beans, short_term, mid_term, long_term, graph_height):
    fig, axes = gridFig([7, 1, 1, 1, 1, 2, 2], (18, 12))
    timestamp = beans.timestamp
    colors = ['gray', 'red', 'green', 'blue']
    labels = ['Close', f'EMA({short_term})-Short', f'EMA({mid_term})-Mid', f'EMA({long_term})-Long']
    plot_prices(axes[0], timestamp, [beans.cl, beans.ema_short, beans.ema_mid, beans.ema_long], colors, labels, graph_height)
   
    axes[1].plot(timestamp, beans.trade_signal, color='blue', label='Trade Signal')
    plot_signal_marker(axes[0], timestamp, beans.trade_signal, beans.cl)
    
    axes[2].plot(timestamp, beans.perfect_order, color='blue', label='Perfect Order')
    draw_bar(axes[2], timestamp, beans.perfect_order)
    
    axes[3].plot(timestamp, beans.trend, color='blue', label='Trend')
    draw_bar(axes[3], timestamp, beans.trend)
    
    axes[4].plot(timestamp, beans.active, color='blue', label='Active')
    
    axes[5].plot(timestamp, beans.atrp, color='red', label='ATRP')
    #draw_bar_level(axes[6], timestamp, beans.atrp, 0.02)
    
    
    axes[6].plot(timestamp, beans.short_slope, color='red', label='EMA-Short slope')
    axes[6].plot(timestamp, beans.mid_slope, color='green', label='EMA-Mid slope')
    beans.long_slope[0] = 0.0
    axes[6].plot(timestamp, beans.long_slope, color='blue', label='EMA-Long slope') 
    draw_bar(axes[6], timestamp, beans.mid_trend)
