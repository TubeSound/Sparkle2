...
from technical import calc_sma, calc_ema, ATRP, is_nans, trend_heikin
...
class Dean:
    def __init__(self, symbol, param: DeanParam):
        self.symbol = symbol
        self.param = param
    ...
    def calc(self, df):
        ...
        self.ema_short = calc_ema(self.cl, self.param.short_term)
        self.ema_upper = self.ema_short + self.param.ema_shift
        self.ema_lower = self.ema_short - self.param.ema_shift
        self.trend = trend_heikin(df, self.param.upper_timeframe, self.param.trend_threshold)
    ...
    def detect_signals(self):
        ...
        for i in range(1, n):
            if self.trend[i] == -1:
                if self.cl[i - 1] <= self.ema_upper[i - 1] and self.hi[i] > self.ema_upper[i]:
                    # Short
                    if state == 0:
                        entries[i] = Signal.SHORT
                        last_price = self.hi[i]
                        state = Signal.SHORT
                    elif state == Signal.SHORT:                    
                        if last_price >= self.hi[i]: 
                            entries[i] = Signal.SHORT
                            last_price = self.hi[i]
                            state = Signal.SHORT 
                        else:
                ...
                if self.cl[i - 1] >= self.ema_lower[i - 1] and self.lo[i] < self.ema_lower[i]:
                    # Long
                    if state == 0:
                        entries[i] = Signal.LONG
                        last_price = self.lo[i]
                        state = Signal.LONG
                    elif state == Signal.LONG:
                        if last_price <= self.lo[i]:
                            entries[i] = Signal.LONG
                            last_price = self.lo[i]
                            state = Signal.LONG
                        else:
